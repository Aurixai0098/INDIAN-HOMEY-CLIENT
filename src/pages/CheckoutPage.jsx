...
const CheckoutPage = () => {
  ...
  // Load direct booking or cart items
  useEffect(() => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // ✅ Payment method 'cod' – no online payment
      const payment = {
        method: 'cod',
        status: 'pending'
      };

      const scheduledTime = { start: '10:00' }; // adjust as needed
      ...
      console.log('📦 Creating booking (no payment):', bookingData);

      const bookingRes = await createBooking(bookingData);
      if (!bookingRes.success) {
        const errorMsg = bookingRes.errors 
          ? bookingRes.errors.map(e => `${e.field}: ${e.message}`).join(', ')
          : bookingRes.message || 'Failed to create booking';
        throw new Error(errorMsg);
      }
      const bookingId = bookingRes.data.booking._id;

      // Clear cart if this was a cart-based checkout
      if (!location.state?.directBooking) {
        clearCart();
      }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link to="/" className="bg-emerald-600 text-white px-6 py-2 rounded-xl hover:bg-emerald-700">
            Browse Services
          </Link>
        </div>
      </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              {addresses.length === 0 && !showAddressForm && (
                <button
                  ...
                  className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-emerald-600 font-medium flex items-center justify-center gap-2"
                >
              ...
              {!showAddressForm && addresses.length > 0 && (
                <div className="space-y-3">
                  ...
                  <button onClick={() => setShowAddressForm(true)} className="text-emerald-600 text-sm font-medium flex items-center gap-1 mt-2">
          ...
          <div className="lg:w-96 space-y-6">
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-6">
              ...
              <button
                ...
                className="w-full mt-6 bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
