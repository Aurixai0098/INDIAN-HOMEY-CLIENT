              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100">
                  <div>
                    ...
                    <span className="text-gray-400 ml-1">/{service.priceUnit?.replace('_', ' ') || 'service'}</span>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {relatedServices.map(rel => (
                    <Link key={rel._id} to={`/service/${rel.slug}`} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition group">
                      <div className="aspect-video bg-gray-100">
                        <img src={rel.images?.[0]?.url || 'https://via.placeholder.com/400x300'} alt={rel.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                      </div>
                      <div className="p-3">
                  ...
                  allCategories.map(cat => (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-4 py-3 hover:bg-emerald-50 transition-colors ${
                        cat._id === service.category?._id ? 'bg-emerald-50 text-emerald-700 font-medium border-l-4 border-emerald-600' : 'text-gray-700'
                      }`}
                    >
                      {cat.icon?.url ? (
