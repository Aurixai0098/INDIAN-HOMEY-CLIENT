...
const AdminProviders = () => {
  ...
  const loadProviders = async () => {
    setLoading(true);
    try {
      const res = await fetchAdminUsers(page, 50);
      if (res.success) {
        ...
        setTotalPages(Math.ceil(filteredProviders.length / 15));
      }
    } catch (err) {
      console.error(err);
  ...
  const viewProviderDetails = async (providerId) => {
    try {
      const res = await fetchAdminUserDetails(providerId);
      if (res.success) {
        setSelectedProvider(res.data.user);
        setShowModal(true);
      }
    } catch (err) {
  ...
  const updateStatus = async (providerId, newStatus) => {
    setStatusUpdating(true);
    try {
      await updateAdminUserStatus(providerId, newStatus);
      loadProviders();
  ...
  const filteredProviders = providers.filter((provider) => {
    ...
    const matchesVerification = verificationFilter === 'all' || provider.isVerified === verificationFilter;
    return matchesSearch && matchesStatus && matchesVerification;
  });

  // Pagination for filtered results
  const paginatedProviders = filteredProviders.slice((page - 1) * 15, page * 15);
  const filteredTotalPages = Math.ceil(filteredProviders.length / 15);

  const activeCount = providers.filter((p) => p.status === 'active').length;
  const suspendedCount = providers.filter((p) => p.status === 'suspended').length;
  const verifiedCount = providers.filter((p) => p.isVerified === true).length;
  const unverifiedCount = providers.filter((p) => p.isVerified !== true).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subtext={`${((activeCount / totalProviders) * 100 || 0).toFixed(0)}% of total`}
        />
        <StatCard
          icon={UserX}
          label="Suspended"
        ...
        <StatCard
          ...
          subtext={`${unverifiedCount} unverified`}
        />
      </div>
              ...
              {paginatedProviders.map((provider) => (
                <tr key={provider._id} className="group hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${provider.status === 'active' ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                        ...
                        {actionMenuOpen === provider._id && (
                          <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-1 z-50 animate-fadeIn">
                            {provider.status === 'active' ? (
                              <button
                                onClick={() => updateStatus(provider._id, 'suspended')}
                              ...
                              <button
                                onClick={() => updateStatus(provider._id, 'active')}
              ...
              <div className="relative flex flex-col items-center text-center">
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold
                    ${selectedProvider.status === 'active' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300'}`}>
                    {selectedProvider.status === 'active' ? <CheckCircle2 className="w-3 h-3" /> : <Ban className="w-3 h-3" />}
                    {selectedProvider.status}
                  </span>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold
                    ${selectedProvider.isVerified ? 'bg-purple-500/20 text-purple-300' : 'bg-amber-500/20 text-amber-300'}`}>
                    <Verified className="w-3 h-3" />
            ...
            <div className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <p className="text-sm font-medium text-slate-800 break-all">{selectedProvider.email}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-slate-500 text-xs font-medium uppercase tracking-wider mb-2">
                  ...
                  <p className="text-sm font-medium text-slate-800">{selectedProvider.phone || 'Not provided'}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-slate-500 text-xs font-medium uppercase tracking-wider mb-2">
                    <Calendar className="w-3.5 h-3.5" />
                ...
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <p className="text-sm font-mono text-slate-600">{selectedProvider._id?.slice(-12)}</p>
                </div>
              </div>
                ...
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProvider.serviceCategories.map((cat, idx) => (
                      <span key={idx} className="px-2 py-1 bg-white rounded-lg text-xs text-slate-600 shadow-sm">
                        {cat}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                {selectedProvider.status === 'active' ? (
                  <button
                    onClick={() => { updateStatus(selectedProvider._id, 'suspended'); setShowModal(false); }}
                  ...
                  <button
                    onClick={() => { updateStatus(selectedProvider._id, 'active'); setShowModal(false); }}
