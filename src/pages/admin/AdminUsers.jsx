    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subtext={`${((activeCount / totalUsers) * 100 || 0).toFixed(0)}% of total`}
        />
        <StatCard
          icon={UserX}
          label="Suspended"
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div className="text-sm text-slate-500">
          Showing <span className="font-semibold text-slate-700">{filteredUsers.length}</span> of <span className="font-semibold text-slate-700">{totalUsers}</span> users
        </div>
      </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{user._id?.slice(-8) || 'N/A'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto animate-scaleIn">
            ...
            <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-8 py-8 text-white overflow-hidden">
              ...
              <div className="relative flex flex-col items-center text-center">
                <UserAvatar name={selectedUser.fullName} size="xl" />
                ...
                <div className="flex items-center gap-2 mt-3">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold
                    ${selectedUser.role === 'admin' ? 'bg-purple-500/20 text-purple-300' :
                      selectedUser.role === 'provider' ? 'bg-blue-500/20 text-blue-300' :
                      'bg-slate-500/20 text-slate-300'}`}>
                    {selectedUser.role === 'admin' && <Crown className="w-3 h-3" />}
                    {selectedUser.role?.charAt(0).toUpperCase() + selectedUser.role?.slice(1)}
                  </span>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold
                    ${selectedUser.status === 'active' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300'}`}>
                    {selectedUser.status === 'active' ? <CheckCircle2 className="w-3 h-3" /> : <Ban className="w-3 h-3" />}
                    {selectedUser.status}
                  </span>
                </div>
              </div>
            ...
            <div className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <p className="text-sm font-medium text-slate-800 break-all">{selectedUser.email}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-slate-500 text-xs font-medium uppercase tracking-wider mb-2">
                  ...
                  <p className="text-sm font-medium text-slate-800">{selectedUser.phone || 'Not provided'}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-slate-500 text-xs font-medium uppercase tracking-wider mb-2">
                    <Calendar className="w-3.5 h-3.5" />
                ...
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <p className="text-sm font-mono text-slate-600">{selectedUser._id?.slice(-12)}</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                {selectedUser.status === 'active' ? (
                  <button
                    onClick={() => { updateStatus(selectedUser._id, 'suspended'); setShowModal(false); }}
                  ...
                  <button
                    onClick={() => { updateStatus(selectedUser._id, 'active'); setShowModal(false); }}
